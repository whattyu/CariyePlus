...
class Collect {
	...
	async buttonCollect(message, userId, yourcard, dealercard, DECK, options) {
		...
		const result = await message.awaitMessageComponent({ filter, time: 30000 })
			.then(async i => {
				switch (i.customId) {
	...
	async messageCollect(message, userId, yourcard, dealercard, DECK, options, filter1) {
		...
		const filter = i => filter1.includes(i.content.toLowerCase()) && i.author.id === userId;
		const result = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] })
			.then(async msg => {
				msg = msg.first();
				if (!msg) {
					if (options.transition === 'edit') {
	...
	async hit(message, userId, yourcard, dealercard, DECK, options) {
		...
		if (options.isSplit === 'first' && options.secondHand) {
			...
			const pv = yourcard.map(c => c.value).reduce((a, b) => b + a);
			if ((pv === 9 || pv === 10 || pv === 11) && yourcard.length == 2) {
				...
				const row1 = { type: 1, components: [hitbtn, standbtn, ddownbtn] };
				...
				else if (options.commandType === 'message') {
			...
			const pv2 = yourcard.map(c => c.value).reduce((a, b) => b + a);
	...
	async stand(message, userId, yourcard, dealercard, DECK, options) {
		const yourvalue = yourcard.map(c => c.value).reduce((a, b) => b + a);
		...
		if (options.isSplit === 'first') {
			...
			const dealerrank = [dealercard[0].rank, dealercard[1].rank];
			...
			const yourvalue2 = finalResult2.ycard.map(c => c.value).reduce((a, b) => b + a);
			const yourcard2 = finalResult2.ycard;
			let bj1 = false;
			let bj2 = false;
			let dbj1 = false;
			let dbj2 = false;
		...
		else if ((dealervalue === 21 && dealercard.length === 2)) {
	...
	async insurance(message, userId, yourcard, dealercard, DECK, options) {
		...
		const dealervalue = dealercard.map(c => c.value).reduce((a, b) => b + a);
